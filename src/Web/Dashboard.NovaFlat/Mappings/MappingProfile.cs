...
namespace Dashboard.NovaFlat.Mappings
{
    public class MappingProfile : Profile
    {
        public MappingProfile()
        {
            ...
            CreateMap<DataTableModel<AnexoVModelFilter, int>, AnexoVListaQuery>()
                .ForMember(dest => dest.id_departamento, opt => opt.MapFrom(src => string.IsNullOrWhiteSpace(src.filter!.departamento) ? 0 : Convert.ToInt32(src.filter.departamento)))
                .ForMember(dest => dest.id_provincia, opt => opt.MapFrom(src => string.IsNullOrWhiteSpace(src.filter!.provincia) ? 0 : Convert.ToInt32(src.filter.provincia)))
                .ForMember(dest => dest.MesFactura, opt => opt.MapFrom(src => Convert.ToInt32(src.filter!.MesFactura)))
                .ForMember(dest => dest.AnioFactura, opt => opt.MapFrom(src => Convert.ToInt32(src.filter!.AnioFactura)))
                .ForMember(dest => dest.NumEvento, opt => opt.MapFrom(src => string.IsNullOrEmpty(src.filter!.NumEvento) ? 0 : Convert.ToInt32(src.filter.NumEvento)))
                .ForMember(dest => dest.Flat, opt => opt.MapFrom(src => Convert.ToInt32(src.filter!.Flat)))
                .ForMember(dest => dest.fechaInico, opt => opt.MapFrom(src => src.filter!.fechaIncio))
                .ForMember(dest => dest.fechaFinal, opt => opt.MapFrom(src => src.filter!.fechaFin))
                .ForMember(dest => dest.clase, opt => opt.MapFrom(src => src.filter!.clase))
                .ForMember(dest => dest.placa, opt => opt.MapFrom(src => string.IsNullOrEmpty(src.filter!.placa) ? "" : src.filter!.placa))
                .ForMember(dest => dest.Factura, opt => opt.MapFrom(src => src.filter!.Factura))
                .ForMember(dest => dest.NumEvento, opt => opt.MapFrom(src => Convert.ToInt32(src.filter!.NumEvento)));

            CreateMap<AnexoVModel, AnexoV>().ReverseMap();
        }
    }
}
